...
This folder is a self-contained, portable bundle for the 10 Oxylabs Dedicated
ISP (static ISP) routes assigned to this account.

## Files

- `proxy-urls.txt` — one authenticated proxy URL per line, ports 8001–8010.
- `proxies.env` — a base `STATIC_ISP_PROXY_URL`, `PROXY_01_URL` through
  `PROXY_10_URL`, and a comma-separated `STATIC_ISP_PROXY_URLS` value.
...
To verify that the 10 ports currently resolve to 10 distinct public IPs:
